...
L.widget.bindClass(L.widget.BaseWidget.extend({
    ...
    activate: function () {
        ...
        for (var i in map.gisdata.controls) {
            map.removeControl(map.gisdata.controls[i]);
        }
        $("#location").hide();
        $("#btnLayerMgr").hide();

        $('#map').removeClass('leaflet-container');
        $('#map').addClass('leaflet-mapbj');

        if (this.layerWork) {
            map.addLayer(this.layerWork);
        }
        map.addLayer(this.layerText);
        map.on("zoomend", this.map_zommendHandler, this);
        this.map_zommendHandler();
    },
    //关闭释放
    disable: function () {
        $('#map').removeClass('leaflet-mapbj');
        $('#map').addClass('leaflet-container');
        for (var i in map.gisdata.controls) {
            map.addControl(map.gisdata.controls[i]);
        }
        $("#location").show();
        $("#btnLayerMgr").show();
        ...
        map.removeLayer(this.layerText);
        map.off("zoomend", this.map_zommendHandler, this);

        if (document.getElementById('legendContainer')) {
            $('#legendContainer').remove();
        } 
    },
    map_zommendHandler: function (e) {
    ...
    addLayer: function (geojson, selectData) {
        ...
        for (var i = 0; i < layers.length; i++) {
            ...
            var size = attr.shortname.length * 15;
            var marker = L.marker(layer.getCenter(), {
                icon: L.divIcon({
                    className: 'leaflet-text-marker leaflet-text-marker-white',
                    html: attr.shortname,
                    iconSize: [size, 15],
                })
            });
            this.layerText.addLayer(marker);
        }
