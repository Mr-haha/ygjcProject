...
L.widget.bindClass(L.widget.BaseWidget.extend({
    ...
    options: {
        ...
        view: {
            ...
            windowOptions: {
                ...
                height: 650,
                position:
                    { left: document.body.clientWidth / 6, top: 0 },
                closeBtn:1
            }
        }
    },
    ...
    ThisIndex: null,
    classObject: null,
    QuhuaURL: "http://10.32.49.27:6080/arcgis/rest/services/QUHUA/MapServer/3",
    JCTBTurl: "http://10.32.49.27:6080/arcgis/rest/services/JCPOLYGON/MapServer/0",
    ...
    doqueryAll: function (index, LXname, div1Obj, tableObj) {
        debugger
        var lquery = L.esri.query({ url: widgetThis.JCTBTurl });
        if (LXname === 'all') {
    ...
    showHistory: function () {
        ...
        if (L.widget.isActivate('widgets/MultiMapCompare/MultiMapCompare.js?TBLX=other')) {
            ...
            L.widget.activate({
                uri: 'widgets/MultiMapCompare/MultiMapCompare.js?TBLX=other', name: '<i class="fa fa-share-alt" style:"font-size:10px"></i>历史追溯' || '', "autoDisable": true, "disableOhter": false
            });
        }
    },
    ...
    queryMapByWhere: function (featurelayerurl, where, isadd, filterdatageojson) {
        ...
        debugger
        lquery.where(where);
        if (featurelayerurl == this.QuhuaURL)
        {
        ...
        if (featurelayerurl == this.JCTBTurl)
        {
            lquery.run(function (error, featureCollection, response) {
            if (featureCollection != null) {
                var resultJson = L.geoJSON(featureCollection, {
    ...
    ZoomToGeometry: function (geometry) {
        if (geometry instanceof L.marker || geometry instanceof L.circle || geometry instanceof L.circleMarker) {
            ...
            map.fitBounds(showExtent, { maxZoom: 15 });
        }
    }
}));
