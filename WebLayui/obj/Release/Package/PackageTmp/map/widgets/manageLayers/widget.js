...
var manageLayersWidget = L.widget.bindClass(L.widget.BaseWidget.extend({
    ...
    getLayers: function () {
        ...
        var layers = [];
        var basemapsCfg = this.hasManagerBaseMaps ? this.map.gisdata.config.basemaps : [];
            ...
            if (item.type == "group" && item.layers) {
                ...
                if (children && children.length > 0) {
                    for (var idx = 0; idx < children.length; idx++) {
                        ...
                        childitem._parent = item;
                        childlayer.config = childitem;
                        childlayer._layer.hasOpacity = true;
                        childlayer._layer.hasZIndex = true;
        ...
        if (parentLayer) {
            if (visible) {
                ...
                parentLayer.eachLayer(function (clayer) {
                    $this.udpateLayerZIndex(clayer, clayer.config.order);
                });
            }
            else
    ...
    udpateLayerOpacity: function (layer, opacity) {
        ...
        if (layer instanceof L.LayerGroup || layer instanceof L.FeatureGroup) {
            layer.eachLayer(function (childlayer) {
                $this.udpateLayerOpacity(childlayer, opacity);
            });
        }
        else {
            if (layer.setOpacity)
