/* 2017-11-6 10:18:04 | 修改 木遥（QQ：346819890） */
//此方式：弹窗非iframe模式
L.widget.bindClass(L.widget.BaseWidget.extend({
    ...
    query: function () {
        ...
        startLatlng = L.mars.pointconvert.wgs2gcj([startLatlng_wgs[1], startLatlng_wgs[0]]);
        endLatlng = L.mars.pointconvert.wgs2gcj([endLatlng_wgs[1], endLatlng_wgs[0]]);

        var str_start_jwd = startLatlng[0] + "," + startLatlng[1];
        var str_end_jwd = endLatlng[0] + "," + endLatlng[1];
        var key = this.getKey();

        haoutil.loading.show('正在分析路线......');
        var $this = this;
        $.ajax({
            ...
            success: function (json) {
                ...
                var data = eval(json);
                if (data.status == 0) {
                    haoutil.alert("请求失败(" + data.infocode + ")：" + data.info);
                    return;
                }

                if (data.route.paths.length == 0) {
                    haoutil.msg("未查询到驾车路线！");
                    return;
                }
                ...
                info.duration = haoutil.str.formatTime(paths0.duration);        //预计行驶时间 
                info.distance = haoutil.str.formatLength(paths0.distance);      //行驶距离 
                info.traffic_lights = paths0.traffic_lights;                    //红绿灯
                if (paths0.tolls > 0) {
                    info.tolls = Number(paths0.tolls).toFixed(0) + "元";              //道路收费
                    info.toll_distance = haoutil.str.formatLength(paths0.toll_distance);   //收费道路长度
                }

                $this.addRouteCar(paths0.steps, info);
    ...
    addRouteCar: function (arrSteps, info) {
        ...
        for (var i = 0; i < arrSteps.length; i++) {
            ...
            $.each(mpts, function (index, mpt) {
                ...
                var lnglat = L.mars.pointconvert.gcj2wgs([jd, wd])
                var result = $this.map.convert2map([lnglat[1], lnglat[0]]);
            ...
            $this.layerWork.addLayer(pointMarker);
            ...
            //添加路线到图层
            var polyline = L.polyline(stepLine, { color: '#3388ff', weight: 5, opacity: 0.8 }).addTo($this.layerWork);
            var remark = (i + 1) + ". " + stepitem.instruction;
            polyline.bindTooltip(remark, { className: 'jsondatalayer-tooltip', direction: 'top' });

            $this.objStepLine[i] = polyline; //记录值，用于定位
        ...
        for (var i in $this.objStepLine) {
            var polyline = $this.objStepLine[i];
