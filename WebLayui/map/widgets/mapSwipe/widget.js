/* 2017-9-27 08:02:34 | 修改 木遥（QQ：346819890） */
//模块：
var mapSwipeWidget = L.widget.bindClass(L.widget.BaseWidget.extend({
    ...
    activate: function () {
        ...
        var $this = this;
        setTimeout(function () {
            $this.updateBaseLayer($this.layerBase);
        }, 500);
    ...
    clipLayer: function (layerSw, rect) {
        ...
        if (layerSw instanceof L.LayerGroup || layerSw instanceof L.FeatureGroup) {
            ...
            layerSw.eachLayer(function (layer) {
                $this.clipLayer(layer, rect);
            });
        }
        else {//图层时
            layerSw.getContainer().style.clip = rect;
        }
    },
    swipeType: "1",
    ...
    mapPageCustomer2: {
        ...
        targetMoveTBHandle: function ($this, obj1, h) {
            var $this = this;
            this.$drag.bind("touchstart", function (event) {
                var height = $this.$parent.width();
                var h = 0; //纠偏值
