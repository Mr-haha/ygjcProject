/* 2017-12-7 13:23:59 | 修改 木遥（QQ：346819890） */
//模块：
    ...
    autoTipList: function (text, queryEx) {
        ...
        $.ajax({
            ...
            data: {
                ...
                "query": text,
                "city_limit": true,
                "region": this.configBaidu.region
            },
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (pois && pois.length > 0 && pois[0].num > 0)
                    $this.workShowCitys.showResult(pois);
                else
                    $this.showPOIPage(pois, data.total);
            },
            error: function (data) {
                haoutil.msg("请求出错(" + data.status + ")：" + data.statusText);
            }
    ...
    showPOIArr: function (arr) {
        ...
        $.each(arr, function (index, item) {
            ...
            if (jd > 0 && wd > 0) {
                ...
                latlng = $this.map.convert2map([wd, jd]);
                ...
                var marker = L.marker(latlng, {
                    ...
                    //})
                    icon: L.icon({
                        "iconUrl": $this.path + "images/poi.png",
                        "iconSize": [16, 16],
                        "iconAnchor": [8, 8],
                        "popupAnchor": [0, -8]
                    })
