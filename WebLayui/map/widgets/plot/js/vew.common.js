/* 2017-7-25 16:43:12 | 修改 木遥（QQ：346819890） */
/*
 * @Author: HuJunyun
 * @Date:   2016-06-21
...
$(function () {
    ...
    $('.mp_tab_tit li').click(function () {
        if ($(this).hasClass('cur') || $(this).hasClass('disabled')) {
            ...
            $this.parent().siblings(".mp_tab_con").children().eq(index).addClass("cur").siblings().removeClass("cur");
...
(function ($) {
    ...
    $.fn.extend({
        ...
        "checkbox": function () {
            return this.each(function () {
                ...
                if ($input.prop("disabled") == true) {
                    $this.addClass("pnui-check-disbaled");
                } else if ($input.prop("checked") == true) {
                    $this.addClass("pnui-checked");
                } else {
                    $this.removeClass("pnui-checked");
                }
                $this.on("click", function () {
                    if ($input.prop("disabled") == true) {
                        return false;
                    } else if ($this.hasClass("pnui-checked")) {
        ...
        "radio": function () {
            return this.each(function () {
                var $this = $(this);
                if ($this.children("input").prop("disabled") == true) {
                    $this.children(".pnui-rdobox").removeClass().addClass("pnui-rdobox pnui-radio-disbaled");
                } else if ($this.children("input").prop("checked") == true) {
                    $this.siblings().children('input').removeAttr("checked");
                    $this.siblings().children(".pnui-rdobox").removeClass("pnui-checked");
                    $this.children(".pnui-rdobox").addClass("pnui-checked");
                } else {
                    $this.siblings().children('input').prop("checked", "checked");
                    $this.siblings().children(".pnui-rdobox").addClass("pnui-checked");
                    $this.children(".pnui-rdobox").removeClass("pnui-checked");
                }
                $this.on("click", function () {
                    var that = $(this);
        ...
        progress: function (max) {
            ...
            return this.each(function () {
                ...
                _html.push("<div class=\"" + opts.progress_text + "\">" + $this.val() + "%</div>");
                _html.push("</div>");
                var pro = $(_html.join(""));
